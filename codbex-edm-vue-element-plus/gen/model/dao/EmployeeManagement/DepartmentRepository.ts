...
export class DepartmentRepository {
    ...
    public customDataCount(): number {
        const resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "DEMO_DEPARTMENT"');
    ...
    private async triggerEvent(data: DepartmentEntityEvent | DepartmentUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-edm-vue-element-plus-EmployeeManagement-Department", ["trigger"]);
        ...
        producer.topic("codbex-edm-vue-element-plus-EmployeeManagement-Department").send(JSON.stringify(data));
    }
}
