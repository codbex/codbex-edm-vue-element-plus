angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-edm-vue-element-plus-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "RecruitmentAndOnboarding" && e.view === "JobPosting" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.filter(e => e.perspective === "RecruitmentAndOnboarding" && e.view === "JobPosting" && e.type === "entity");
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("JobPosting", `Unable to count JobPosting: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("JobPosting", `Unable to list/filter JobPosting: '${response.message}'`);
						return;
					}

					response.data.forEach(e => {
						if (e.PostingDate) {
							e.PostingDate = new Date(e.PostingDate);
						}
						if (e.ApplicationDeadline) {
							e.ApplicationDeadline = new Date(e.ApplicationDeadline);
						}
					});
		...
		$scope.openDetails = function (entity) {
			$scope.selectedEntity = entity;
			messageHub.showDialogWindow("JobPosting-details", {
				action: "select",
				entity: entity,
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("JobPosting-filter", {
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				'Delete JobPosting?',
				`Are you sure you want to delete JobPosting? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("JobPosting", `Unable to delete JobPosting: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);
