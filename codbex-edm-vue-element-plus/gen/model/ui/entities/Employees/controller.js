angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', 'messageHub', 'entityApi', 'Extensions', function ($scope, messageHub, entityApi, Extensions) {
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("Employees", `Unable to count Employees: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Employees", `Unable to list/filter Employees: '${response.message}'`);
						return;
					}
		...
		$scope.openDetails = function (entity) {
			$scope.selectedEntity = entity;
			messageHub.showDialogWindow("Employees-details", {
				action: "select",
				entity: entity,
			});
		};
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("Employees-filter", {
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				'Delete Employees?',
				`Are you sure you want to delete Employees? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("Employees", `Unable to delete Employees: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);
